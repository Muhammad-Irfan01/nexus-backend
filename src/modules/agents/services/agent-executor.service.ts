...
export class AgentExecutorService {
  ...
  async executeAgent(agentId: string, userId: string, message: string) {
    ...
    const contentResponse = await this.retrivalservice.retrive(
      message,
      5,
      workspaceDocumentIds,
    );
    const results = Array.isArray(contentResponse)
      ? contentResponse
      : contentResponse?.points || [];
    ...
    await this.usageTracker.track(userId, agent.workspaceId, 'AGENT_MESSAGE', {
      agentId,
      message,
    });
